...
public class SpiderFlowThreadPoolExecutor {
    ...
    public class SubThreadPoolExecutor {
        ...
        private void submit() {
            while (running) {
                //循环从taskList中获取SpiderFutureTask交给executor来执行
                try {
                    synchronized (submitStrategy) {
                        //如果集合是空的，则等待提交
                        if (submitStrategy.isEmpty()) {
//                            log.info("submitStrategy.isEmpty(),waiting...");
    ...
    public static void main(String[] args) throws Exception {
        ...
        SpiderFlowThreadPoolExecutor executorInstance = new SpiderFlowThreadPoolExecutor(5);
        ThreadSubmitStrategy strategy = new LinkedThreadSubmitStrategy();//RandomThreadSubmitStrategy();
        SubThreadPoolExecutor pool = executorInstance.createSubThreadPoolExecutor(1, strategy);
        for (int i = 0; i < 10; i++) {
            int finalI = i;
            try {
                Thread.sleep(1000);
            ...
            pool.submitAsync(() -> {
                log.info("current:{} index:{}", Thread.currentThread().getName(), finalI);
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
